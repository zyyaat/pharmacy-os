...
func executeWithCompanyTenantContext(
	...
	// Check if response was already written (abort called)
	if c.Writer.Written() {
		// Something went wrong in handler, rollback
		_ = tx.Rollback(ctx)
		return errors.New("response already written, rolling back")
	}
...
func setCompanyTenantContextVariables(
	ctx context.Context,
	tx interface{ Exec(ctx context.Context, sql string, arguments ...interface{}) (commandTag interface{}, err error) },
	companyID string,
	userID string,
	isSuperAdmin bool,
) error {
