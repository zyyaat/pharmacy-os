...
func (h *CompanyHandler) Login(c *gin.Context) {
	...
	// Build response
	response := models.CompanyAuthResponse{
		User: middleware.CompanyUserInfo{
			ID:          user.ID,
			Email:       user.Email,
			FirstName:   user.FirstName,
			LastName:    user.LastName,
			DisplayName: func() string { if user.DisplayName != nil { return *user.DisplayName } return "" }(),
...
func (h *CompanyHandler) ForgotPassword(c *gin.Context) {
	...
	token, err := middleware.GenerateSecureToken(32)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
			"error":   "token_generation_failed",
